@extends('layouts.voluntario')

@section('title', 'Evaluación de Voluntario - GEVOPI')
...
<style>
  ...
  .seleccion-cuerpo-box label {
    ...
    line-height: 1.8;
    border-bottom: 2px solid var(--color-texto-principal);
    display: block;
    margin-bottom: 12px;
  }
...
@section('content')
    <div class="form-container">
      <div class="form-content">
        ...
        {{-- Info del voluntario --}}
        <div class="voluntario-info-card">
          <div class="voluntario-avatar">
            {{ strtoupper(substr($voluntario->nombres, 0, 1)) }}
          </div>
          <div class="voluntario-details">
            <h3>{{ $voluntario->nombres }} {{ $voluntario->apellidos }}</h3>
            <p>{{ $voluntario->email ?? 'Voluntario' }}</p>
          </div>
          <a href="#" onclick="cerrarSesionVoluntario(event)" class="btn-salir-evaluacion">
        ...
        <div class="form-header">
          <h1 class="form-titulo">Evaluación de Voluntario</h1>
        </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content text-center">
          ...
          <div class="modal-body">
            ...
            <p>Su evaluación ha sido procesada exitosamente.</p>
            <p class="text-muted">Tu participación es muy importante para nosotros.<br>Serás redirigido en unos segundos...</p>
          </div>
          <div class="modal-footer border-0 justify-content-center">
            <a href="https://www.google.com" class="btn btn-lg text-white" id="btnSalirExito" style="background-color: #353b41;">
    ...
    <div class="modal fade" id="modalConexionError" tabindex="-1" role="dialog">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content text-center">
          ...
          <div class="modal-body">
            ...
            <p id="conexionErrorMessage">No se pudo enviar la evaluación. Por favor, intente nuevamente.</p>
          </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content text-center">
          <div class="modal-body py-5">
            ...
            <p class="mt-3 mb-0">Procesando evaluación...</p>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function closePopup() {
    document.querySelectorAll('.popup-selector').forEach(n => n.remove());
  }
  ...
  async function enviarEvaluacion() {
    ...
    const payload = {
      resumen_fisico: evaluacionFisica,
      resumen_emocional: evaluacionEmocional,
      estado_general: 'Completado',
      observaciones: ''
    };
    
    try {
      const response = await fetch(`/evaluacion-voluntario/${token}/procesar`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.success) {
        ...
        document.getElementById('conexionErrorMessage').textContent = 
          data.message || 'Error al procesar la evaluación.';
