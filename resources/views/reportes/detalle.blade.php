...
@section('title', 'Resultado de Encuesta #' . $reporte->id . ' - ' . ucfirst($tipo))

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h1>Resultado de Encuesta #{{ $reporte->id }} - {{ $tipo == 'fisico' ? 'Evaluación Física' : 'Evaluación Emocional' }}</h1>
            <p class="text-muted mb-0">Fecha realizada: {{ \Carbon\Carbon::parse($reporte->fecha_generado)->format('j/n/Y') }}</p>
        </div>
        <a href="{{ url()->previous() }}" class="btn text-white" style="background-color: #007bff;">
...
<style>
    ...
    /* ======= Lienzo del cuerpo (copiado exactamente de evaluacion.blade.php) ======= */
    .human-body {
        width: 207px;
        position: relative;
        height: 550px;
    ...
    /* Estados de color - colores exactos de evaluacion.blade.php */
...
<div class="row justify-content-center">
    @if($tipo == 'fisico')
        ...
        {{-- Fila 1: Preguntas Físicas (col-8) + Panel Info (col-4) --}}
        <div class="col-md-8 d-flex">
            <div class="card w-100">
                <div class="card-header" style="background-color: #f8f9fa;">
                    <h3 class="seccion-titulo m-0"><i class="fas fa-heartbeat mr-2"></i>Preguntas Físicas</h3>
                </div>
                <div class="card-body">
                    <div class="row">
                        @if(count($respuestasFisicas) > 0)
                            @foreach($respuestasFisicas as $respuesta)
                                <div class="col-md-6">
                                    <div class="pregunta-card">
                                        <div class="pregunta-texto">{{ $respuesta['pregunta'] }}</div>
                                        <div class="respuesta-texto">Resultado: {{ $respuesta['respuesta'] }}</div>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            @php
                                $preguntasFisicas = [
                                    ...
                                    '¿Has notado quemaduras, irritación o enrojecimiento en la piel?',
                                    '¿Tienes dificultad para respirar profundamente?',
                                    '¿Tu nariz está congestionada o bloqueada más de lo normal?',
                                ];
                            @endphp
                            @foreach($preguntasFisicas as $pregunta)
                                <div class="col-md-6">
                                    <div class="pregunta-card">
                                        <div class="pregunta-texto">{{ $pregunta }}</div>
                                        <div class="respuesta-texto">Resultado: Sin respuesta</div>
                                    </div>
                                </div>
                            @endforeach
                        @endif
        ...
        {{-- Panel Derecho: Universidad + Info Reporte --}}
    ...
    @else
        ...
        {{-- Columna Preguntas Emocionales --}}
        <div class="col-md-8 d-flex">
            <div class="card w-100">
                <div class="card-header" style="background-color: #f8f9fa;">
                    <h3 class="seccion-titulo m-0"><i class="fas fa-brain mr-2"></i>Preguntas Emocionales</h3>
                </div>
                <div class="card-body">
                    <div class="row">
                        @if(count($respuestasEmocionales) > 0)
                            @foreach($respuestasEmocionales as $index => $respuesta)
                                <div class="col-md-6">
                                    <div class="pregunta-card">
                                        <div class="pregunta-texto">{{ $respuesta['pregunta'] }}</div>
                                        <div class="respuesta-texto">Resultado: {{ $respuesta['respuesta'] }}</div>
                        ...
                        @else
                            @php
                                $preguntasPsicologicas = [
                                    ...
                                    '¿Has tenido dificultad para concentrarte en tus actividades diarias desde la intervención?',
                                    '¿Has sentido irritabilidad o cambios de humor después de la intervención?',
                                    '¿Has experimentado pesadillas o problemas para dormir relacionados con la intervención?',
                                    '¿Has sentido ansiedad o nerviosismo desde la intervención?',
                                ];
                            @endphp
                            @foreach($preguntasPsicologicas as $index => $pregunta)
                                <div class="col-md-6">
                                    <div class="pregunta-card">
                                        <div class="pregunta-texto">{{ $pregunta }}</div>
                                        <div class="respuesta-texto">Resultado: Sin respuesta</div>
        ...
        {{-- Columna Derecha para vista emocional --}}
        <div class="col-md-4 d-flex flex-column">
            <div class="card mb-3">
                <div class="card-header" style="background-color: #f8f9fa;">
                    <h3 class="seccion-titulo m-0">Universidad Asignada</h3>
            ...
            <div class="card flex-grow-1">
                ...
                <div class="card-body d-flex flex-column justify-content-center">
                    <p><strong>ID Reporte:</strong> #{{ $reporte->id }}</p>
                    <p><strong>Tipo:</strong> 
                        <span class="badge" style="background-color: #17a2b8; color: white;">Evaluación Emocional</span>
                    </p>
                    <p><strong>Fecha:</strong> {{ \Carbon\Carbon::parse($reporte->fecha_generado)->format('j/n/Y H:i') }}</p>
                    <p><strong>Estado General:</strong> 
                        <span class="badge" style="background-color: #007bff; color: white;">{{ $reporte->estado_general ?? 'Completado' }}</span>
...
{{-- Modal Asignar Universidad --}}
<div class="modal fade" id="modalAsignarUniversidad" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #007bff; color: white;">
                <h5 class="modal-title"><i class="fas fa-university"></i> Asignar Universidad</h5>
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label for="universidad_id">Seleccionar Universidad</label>
